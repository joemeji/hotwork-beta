...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { shipping_id, shipping_item_id } = req.query;

  const searchQuery = toStringUrlSearchQuery(req.url);

  const response = await fetch(baseUrl + `/api/projects/shipping/set/` + shipping_id + `/items/${shipping_item_id}` + searchQuery, {
    headers: { 
