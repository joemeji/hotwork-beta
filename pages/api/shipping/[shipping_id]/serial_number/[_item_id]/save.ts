...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { shipping_id, _item_id } = req.query;

  const searchQuery = toStringUrlSearchQuery(req.url);
  
  const response = await fetch(baseUrl + `/api/projects/shipping/items/${shipping_id}/save_serial_numbers/${_item_id}` + searchQuery, {
    headers: { 
